...
int main()
{
    ...
    //Set the work item and work group sizes
    int wi_size = 2048;
    int gp_size = 16;
...
#error "Unknown OpenCL SDK environment"
    ...
    /* Allocate space for pi buffer on the device */
    ...
    /* Copy the pi output data back to the host */
    clEnqueueReadBuffer(command_queue, pi_buf, CL_TRUE, 0, wi_size * sizeof(float), (void *)pi_res, 0, NULL, NULL);

    /* Add all work item results together and multiple sum by 4 */
